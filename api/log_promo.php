...
try {
    if ($method === 'POST') {
        ...
        $stmt->bind_param("issss", $user_id, $input['title'], $input['description'], $start_date, $input['expiry_date']);
        ...
        $stmt->bind_param("ssssii", $input['title'], $input['description'], $start_date, $input['expiry_date'], $input['promo_id'], $user_id);
        ...
        if ($action === 'get_clients' && $promo_id > 0) {
            ...
            // ==========  هذا هو التحديث: جلب كود الدولة مع الرقم  ==========
            ...
            $all_clients_result = $all_clients_stmt->get_result();
            // جلب النتائج كمصفوفة من الكائنات، بدلاً من تجاهل كود الدولة
            $all_clients = $all_clients_result->fetch_all(MYSQLI_ASSOC);
            ...
            foreach ($all_clients as $client) {
                // الآن $client هو كائن كامل يحتوي على (owner_phone, owner_name, owner_phone_code)
                if (!isset($sent_phones[$client['owner_phone']])) { 
                    $clients_to_send[] = $client; // إضافة الكائن الكامل
                }
            }
